...
procedure Formatar(Obj: TObject; Formato : TFormato; Extra : string = ''); overload;
function Formatar(avalue:string;Formato:TFormato; Extra : string = ''):string; overload;
...
function Formatar(avalue:string;Formato:TFormato; Extra : string = ''):string;
begin
 case avalue.Length of
  14: begin
      avalue:=avalue.Remove(0,3);
       result:=Mask('(##) #####-####', SomenteNumero(avalue));
      end;
  else
       result:=Mask('(##) #####-####', SomenteNumero(avalue));
 end;
